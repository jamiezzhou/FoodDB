...
app.post("/search", function (req, res) {
    var restaurant = req.body.resName;
    var type = req.body.types
    if (restaurant != "") {
        var get_res = 'SELECT * FROM restaurant R WHERE R.name = ?';
        connection.query(get_res, restaurant, function (error, result) {
            if (error) throw error;
            else {
                var num_rest = result.length;
                var result_str = '';
                for (let i = 0; i < num_rest; i++) {
                    var str = '\t\t\t\t\t\tRestaurant #' + (i + 1) +
                        '\nRestaurant ID:  ' + result[i].id +
                        '\nRestaurant Name: ' + result[i].name +
                        '\nRestaurant Address: ' + result[i].address +
            ...
            if (error) throw error;
                ...
                for (let i = 0; i < num_rest; i++) {
                    var str = '\t\t\t\t\t\tRestaurant #' + (i + 1) +
                        ...
                        '\nAvailable Seats: ' + result[i].avail_seats + '\n\n';
                    result_str += str;
                }
                res.render("display", { data: result_str });
            }
